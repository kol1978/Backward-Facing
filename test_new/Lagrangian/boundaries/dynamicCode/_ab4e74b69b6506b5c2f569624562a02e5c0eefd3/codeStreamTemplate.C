...
extern "C"
{
    void codeStream_ab4e74b69b6506b5c2f569624562a02e5c0eefd3
    (
        Ostream& os,
    ...
    {
//{{{ begin code
        #line 92 "/home/kol/OpenFOAM/OpenFOAM-14/test/Lagrangian/boundaries/system/blockMeshDict!#codeStream"
                ...
                for (label xi = 0; xi < 4; xi += 2)
                {
                    os  << "hex "
                        << FixedList<label, 8>({
                            ...
                            (yi + 1)*4+xi})
                        << zoneNames[yi]
                        << Vector<label>(nX, nYs[yi], 1)
                        << "simpleGrading" << Vector<label>(1, 1, 1);
                }
            }
