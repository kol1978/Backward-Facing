...
В багрепорте сказано, что та же проблема затрагивает QGDsolver. Это правдоподобно — любой код, использующий copy-initialization от выражений, возвращающих `tmp<GeometricField<...>>` или `tmp<Field<...>>`, будет давать ту же ошибку на Clang/ICX. Но в найденных источниках конкретно `leastSquaresStencil.C` не упоминается — это собственный опыт автора.
...
Так вы избежите ситуации, когда скрипт «отработал», но ничего не заменил.
