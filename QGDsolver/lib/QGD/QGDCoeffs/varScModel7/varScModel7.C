...
namespace qgd
{
    defineTypeNameAndDebug(varScModel7,0);
...
void Foam::qgd::
varScModel7::correct(const Foam::QGDThermo& qgdThermo)
{
    ...
    forAll(ScQGD_, icell)
    {
        ...
        sumpf /= n;
        ScQGD_[icell] = cSc1_*(mag(sumDpF) / sumpf);
    }
    
    if (minSc_ >= 0)
    {
        this->ScQGD_ = max(this->ScQGD_, minSc_);
    }
    if (maxSc_ >= 0)
    {
        this->ScQGD_ = min(this->ScQGD_, maxSc_);
    }
