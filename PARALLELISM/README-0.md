#-----------------------------Часть 1: SIMD vs OpenMP — полный анализ
...
### Почему CFD-сообщество жалуется на AVX-512Главный сюрприз: -frounding-math

Это довольно известная боль в CFD-сообществе — люди покупают Xeon Gold с
AVX-512, компилируют OpenFOAM с `-xCORE-AVX512`, ждут двукратного ускорения,
а получают 10–15%. И потом пишут на CFD Online: «AVX-512 не работает в
OpenFOAM, что я делаю не так?»

Ответ один: структура данных OpenFOAM (`lduAddressing` с косвенной
адресацией) просто не даёт компилятору векторизовать то, что съедает 90%
времени. И это не баг — это архитектурное решение OpenFOAM, заточенное под
гибкость работы с произвольными неструктурированными сетками.
...
#-----------------------------------------------Часть 2: Аудит флагов компиляции
...
| Деление через обратную величину | заблокирована | заблокирована | **заблокирована** |
| Constant folding (вычисление констант) | разрешена | заблокирована | **заблокирована** |
...
| Флаг | Текущее значение | Документация Intel/LLVM | Вердикт |
|---|---|---|---|
| `-O3` | есть | Базовый уровень оптимизации | ✅ Корректно |
| `-march=westmere` | есть | Включает SSE4.2 для Westmere | ✅ Корректно |
| `-fiopenmp` | есть | Intel OpenMP runtime, рекомендуется | ✅ Корректно |
| `-fp-model precise` | есть | Запрещает value-unsafe FP-оптимизации | ⚠️ Узкое место №1 |
| `-frounding-math` | есть | = компонент `-fp-model strict`, блокирует reordering + constant folding + FMA | ❌ Узкое место №2 |
| `-no-prec-div` | нет | Заменяет деление на умножение обратной величиной | ❌ Отсутствует |
| `-fp-model fast=2` | нет | Разрешает агрессивные FP-оптимизации | ❌ Отсутствует |
| `-funroll-loops` | нет | Развёртка циклов | ❌ Отсутствует |
| `-std=c++14` | есть | OpenFOAM v2306 поддерживает C++17 | ⚠️ Можно повысить до C++17 |
| `-fPIC` | есть | Position-independent code (нужно для shared libs) | ✅ Корректно |
| `-pthread` | есть | Threading support | ✅ Корректно |

---

## Откуда взялся `-frounding-math`

OpenFOAM добавляет `-frounding-math` в своих `wmake`-правилах по умолчанию. Это сделано для гарантии воспроизводимости — чтобы результаты не зависели от оптимизаций компилятора. Для научных расчётов, где важна бит-в-бит воспроизводимость, это разумно. Но для производительности — это **убийца оптимизаций**.
...
#-------------------------------------------------Главный сюрприз: -frounding-math
Главный сюрприз: -frounding-math
Это самый критичный флаг в текущей сборке, и он работает против производительности.

Согласно документации Clang/LLVM (на котором построен ICX):
...
Из патча LLVM D62731:

This will make -frounding-math synonymous with -fp-model=strict.

Из маппинга компиляторных флагов xmake:

text
strict = {"-frounding-math", "-ftrapping-math"}
То есть -frounding-math — это компонент -fp-model strict, самого жёсткого режима. Он строже, чем -fp-model precise.
...
FMA (fused multiply-add)	разрешён	заблокирован	заблокирован
Автовекторизация FP-циклов	ограничена	дополнительно ограничена	сильно ограничена
По документации Intel, -fp-model strict = -fp-model precise + запрет FMA

не предполагать default FP environment. Связка -fp-model precise + -frounding-math практически эквивалентна -fp-model strict — самому медленному режиму из всех.
Полный аудит флагов
Флаг	Текущее	Документация	Вердикт
-O3	есть	Базовый уровень оптимизации	✅ Корректно
-march=westmere	есть	Включает SSE4.2 для Westmere	✅ Корректно
-fiopenmp	есть	Intel OpenMP runtime, рекомендуется	✅ Корректно
-fp-model precise	есть	Запрещает value-unsafe FP-оптимизации	⚠️ Узкое место №1
-frounding-math	есть	= компонент -fp-model strict	❌ Узкое место №2
-no-prec-div	нет	Заменяет деление на умножение обратной величиной	❌ Отсутствует
-fp-model fast=2	нет	Разрешает агрессивные FP-оптимизации	❌ Отсутствует
-funroll-loops	нет	Развёртка циклов	❌ Отсутствует
-std=c++14	есть	OpenFOAM v2306 использует C++14	⚠️ Корректно для v2306
-fPIC	есть	Position-independent code	✅ Корректно
-pthread	есть	Threading support	✅ Корректно
Откуда взялся -frounding-math
OpenFOAM добавляет -frounding-math в wmake-правилах по умолчанию. Это сделано для гарантии воспроизводимости — чтобы результаты не зависели от оптимизаций компилятора. Для научных расчётов, где важна бит-в-бит воспроизводимость, это разумно. Но для производительности — это убийца оптимизаций.
...
### Почему CFD-сообщество жалуется на AVX-512
