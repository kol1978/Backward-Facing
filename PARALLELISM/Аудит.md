...
## Часть 4. Аудит флагов компиляции: главный сюрприз — `-frounding-math`
...
### 4.2. Что делает связка `-fp-model precise` + `-frounding-math`
...
### 4.3. Что говорит документация

Согласно Clang User's Manual (на котором построен ICX):
...
> «I'll remove the FIXME and assert that frounding-math uses dynamic-rounding and strict exception behavior. This will make frounding-math synonymous with fp-model=strict.»

То есть `-frounding-math` — это компонент `-fp-model strict`, самого жёсткого режима. Он строже, чем `-fp-model precise`.

### 4.4. Полный аудит флагов
...
### 4.5. Откуда берётся `-frounding-math` и где он встречается
...
1. **CGAL-зависимые утилиты** — `surfaceFeatureExtract/Allwmake` явно передаёт `EXE_FROUNDING_MATH=-frounding-math`, потому что CGAL использует динамическое изменение режима округления для точной геометрической арифметики.
...
3. **Другие архитектуры** — в некоторых архитектурно-специфичных правилах (например, `linuxARM64Arm/c++Opt` в ESI fork) используется `-ffast-math`, который включает `-fno-rounding-math` (обратный флаг).

Проверить происхождение в конкретной сборке:
