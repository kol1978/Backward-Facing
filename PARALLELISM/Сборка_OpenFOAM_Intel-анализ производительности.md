...
**Вывод:** На идеальных циклах AVX-512 рвёт SSE4.2 в 2–4×. Но в OpenFOAM v2306 примерно 90% времени съедают GAMG и smoothSolver — операции с косвенной адресацией и рекуррентными зависимостями, которые не векторизуются ни одним SIMD. Поэтому SIMD-преимущество AVX-512 над SSE4.2 в OpenFOAM — всего ~6–11%.

**OpenMP на Westmere** компенсирует этот разрыв за счёт того, что он параллелит именно те циклы, которые SIMD не берёт — разреженную алгебру GAMG. В итоге `-march=westmere -fiopenmp` дают примерно тот же прирост, что `-xCORE-AVX512` на Xeon Gold — не потому что Westmere хорош, а потому что OpenFOAM плохо векторизуется, и увеличение ширины SIMD‑векторов (переход от SSE4.2 к AVX‑512, то есть от 128‑ до 512‑битных регистров) почти не даёт прироста производительности — потому что большинство «тяжёлых» циклов в OpenFOAM вообще нельзя векторизовать.
...
# Дополнение
Статья «Инструменты Intel для оптимизации» (habr.com/ru/companies/intel/articles/310842/) — контекст: векторизация CFD-подобного кода
Статья описывает оптимизацию задачи о течениях в пористых средах (CFD-подобный код, не OpenFOAM, но схожая структура):
«Большая часть циклов была автоматически векторизована компилятором... производительность после автоматической векторизации выросла примерно в три раза» «Использование опций -ipo, -xHost и -no-prec-div общее время исполнения программы снизилось ещё сильнее, до 53.5 секунд»
Это подтверждает, что для CFD-кода -no-prec-div даёт заметный прирост, а векторизация эффективна только для структурированных циклов — но в OpenFOAM с lduAddressing структурированных циклов почти нет.

Статья «OpenFOAM с точки зрения программиста-физика» (habr.com/ru/companies/intel/articles/170675/) — подтверждает архитектурный контекст
Статья описывает в том числе:
«Цепочки шаблонов... constTransport<specieThermo<hConstThermo>>» «в OpenFOAM нет поддержки комплексных чисел» «процедура сборки, установки и настройки... требует прописывания вручную в файле настроек пути»
Это подтверждает, что OpenFOAM имеет сложную template-архитектуру, которая (в v2606 с expression templates) стала ещё сложнее — и именно эти сложные типы ломают OpenMP mapping.

Статья «Как оптимизировать код на С для x86-процессоров» (habr.com/ru/companies/yadro/articles/779284/) — подтверждает cache split и alignment
Статья от Yadro подтверждает:
«Для AVX-512 любое чтение по адресу, не кратному 64-м, затрагивает две кэш-линии (cache split)» «AVX512-код обгоняет AVX2 почти в два раза» (в L1) «Данные в L2... производительность сравнялась» «Данные в L3... производительность всех трёх версий кода снова на равных»
Это косвенно подтверждает тезис о том, что AVX-512 преимущество быстро исчезает при выходе за пределы L1 — а OpenFOAM с косвенной адресацией именно это и делает (обращается к данным по произвольным индексам, разрушая cache locality).

Статья «Оптимизируем шаг за шагом с компилятором Intel C++» (habr.com/ru/companies/intel/articles/256251/) — подтверждает состав -fast
Статья прямо перечисляет:
«для ленивых придумали опцию -fast, которая включает в себя большинство этих ключей: ipo, -O3, -no-prec-div, -static, -fp-model fast=2, and -xHost»
Также подтверждает:
-O3 включает «более агрессивные трансформации с циклами, например, развёртку (unrolling) внешнего цикла и объединение (fusion) внутренних»
-no-prec-div — самостоятельный флаг, не входящий в -O3
-xHost — оптимизация под железо, на котором собирается код

Статья «Балансируя между точностью и производительностью» (habr.com/ru/companies/intel/articles/160747/) — подтверждает ключевое утверждение о FMA
Это статья от Intel на Хабре, которая прямо подтверждает таблицу fp-model режимов. Из неё:
Режим precise: FMA = да, безопасность значений = да, окружение = нет, исключения = нет Режим strict: FMA = нет, безопасность значений = варьируется, окружение = да, исключения = да Режим fast=1 (по умолчанию): FMA = да, безопасность значений = небезопасно Режим fast=2: FMA = да, безопасность значений = небезопасно
Это прямо подтверждает исправленное утверждение в тексте: -fp-model precise не отключает FMA. Только strict его отключает. Связка -fp-model precise + -frounding-math не эквивалентна strict по FMA.

Тонкость: дефолт — не -no-prec-div, а «середина»
Это ключевой нюанс, который часто упускают. Документация Intel:
Режим	Точность деления	Скорость
-prec-div	Полная IEEE-точность	Самый медленный
Default (без флага)	«Середина» — не IEEE, но точнее -no-prec-div	Средний
-no-prec-div	Наименее точный (A/B → A*(1/B))	Самый быстрый
Презентация CERN по FP control:
Default is -no-prec-sqrt, and somewhere between -prec-div and -no-prec-div

То есть дефолт — это не -no-prec-div, а промежуточный режим. Явное указание -no-prec-div даёт дополнительный прирост по сравнению с дефолтом, потому что заменяет деление на умножение на обратную величину — а дефолт этого не делает (или делает менее агрессивно).

Иначе говоря:
-fp-model precise → неявно включает -prec-div (точное деление)
Явный -no-prec-div → переопределяет неявный -prec-div от -fp-model precise
Результат: остальные FP-операции остаются «precise», но деление становится быстрым
Это означает, что добавление -no-prec-div к -fp-model precise — легитимная комбинация: она не противоречит, а точечно переопределяет одну FP-операцию (деление), сохраняя точность для остальных.

# Уточнения к тексту
На основе сверки с Хабр-публикациями можно внести следующие уточнения:
В разделе о downclocking — указать, что «30–40%» — это диапазон для Skylake-SP, подтверждённый конкретными замерами (например, Xeon Silver 4116: 33%), и что Ice Lake снижает это до ~100 МГц [habr.com/ru/articles/517628/].
В разделе о -fp-model — добавить прямую ссылку на Хабр-статью Intel, которая в таблице прямо показывает FMA=да для precise [habr.com/ru/companies/intel/articles/160747/].
В разделе о -fast метафлаге — подтвердить состав через Хабр-статью [habr.com/ru/companies/intel/articles/256251/].
Оценку «10–15%» AVX-512 в OpenFOAM — отметить как оценку, основанную на IXPUG и MG-CFD, а не как прямой результат Хабр-публикации.
